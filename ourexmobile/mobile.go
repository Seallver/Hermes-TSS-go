...
func (p *MPCDotLib) Sign(p2p []string, id, threshold int, keyShare []byte, path string, messageToSign []byte) ([]byte, []byte, byte, error) {
	...
	sig, err := HermesSign1(bipCfg, messageToSign, signers, network, pl)
	if err != nil {
		return nil, nil, 0, err
	}
...
func (p *MPCDotLib) RefreshKey(p2p []string, id, threshold int, keyShare []byte) ([]byte, error) {
	...
	cfgNew, err := HermesRefresh1(cfgOld, network, pl)
	if err != nil {
...
func (p *MPCDotLib) GenKey(p2p []string, id, threshold int) ([]byte, error) {
	...
	cfg, err := HermesKeygen1(uid, ids, threshold, network, pl)
	if err != nil {
		return nil, err
	}
...
func HermesSign1(c *Hermes.Config, m []byte, signers party.IDSlice, n test.INetwork, pl *pool.Pool) (*ecdsa.Signature, error) {
